...
@app.get("/debug/dealer-matching/{ticket_id}")
async def debug_dealer_matching(ticket_id: str):
    """Debug dealer matching logic step by step"""
    try:
        from enhanced_zoho_integration import ZohoTicketFetcher
        ...
        # Test dealer matching with detailed logging
        dealer_name, dealer_id, rep_name = extract_dealer_from_text(
            full_text, subject, oem, from_email
        )
        ...
        return {
            ...
            "text_analysis": {
                "contains_evan_walsh": "evan walsh" in full_text.lower(),
                "contains_belliveau": "belliveau" in full_text.lower(),
                "contains_ford": "ford" in full_text.lower(),
                "potential_dealer_ids": re.findall(r'\b\d{4}\b', full_text),
                "potential_names": re.findall(r'\b[A-Z][a-z]+ [A-Z][a-z]+\b', full_text)
            }
        }
    except Exception as e:
        return {"error": str(e)}
